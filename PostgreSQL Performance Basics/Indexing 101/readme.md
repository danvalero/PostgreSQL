...
1. Query the table filtering by a column not included in the Primary Key
   ...
   **Bitmap Index Scan** as explained at [Tom Lane’s post to the PostgreSQL performance mailing list](https://www.postgresql.org/message-id/12553.1135634231@sss.pgh.pa.us)
   ...
   if you get an execution plan that uses  Bitmap Index Scan +  Bitmap Heap Scan, PostgreSQL is validating the table's **Visibility Map**. That is a topic for another post.
   ...
   -------------------------------------------------------------------------------------------------------
    Bitmap Heap Scan on customer  (cost=2.29..5.37 rows=2 width=49) (actual time=0.032..0.033 rows=2.00 loops=1)
      ...
      ->  Bitmap Index Scan on ix_customer_first_name  (cost=0.00..2.29 rows=2 width=0) (actual time=0.021..0.021 rows=2.00 loops=1)
            Index Cond: (first_name = 'LESLIE'::text)
            Index Searches: 1
            Buffers: shared read=2
   ```

   in that case, you can VACCUM the table

   ```sql
   VACUUM ANALYZE public.customer;
   ```
   ...
   QUERY PLAN
   -------------------------------------------------------------------------------------------------------
   Index Only Scan using ix_customer_first_name on customer  (cost=0.28..2.31 rows=2 width=49) (actual time=0.009..0.010 rows=2.00 loops=1)
     Index Cond: (first_name = 'LESLIE'::text)
     Heap Fetches: 0
     Index Searches: 1
     Buffers: shared hit=3
